riot.tag('todo-app', '<div class="add"> <div class="add__left"> <input class="add__input" name="add"> </div> <div class="add__right"> <button class="add__submit">Add Todo</button> </div> </div> <ul class="{ block + \'__list\' }" if="{ todos.length }"> <li class="{ block + \'__item\' }" each="{ todos }"> <label><input class="checkbox" type="checkbox" __checked="{ completed }">{ name }</label> </li> </ul> <div class="{ block + \'__foot\' }"> <ul> <li class="{ block + \'__info\' }"> { getLeftString() } </li> <li><a href="">Mark all as complete</a></li> </ul> </div>', function(opts) {
    this.block = 'todo-app';
